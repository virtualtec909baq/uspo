class Api::RemittentsController < ApplicationController
  ...
  def index
    ...
    @remittents.each do |remittent|
      a = ["id", "#{remittent.id}", "user_id", "#{remittent.user_id}", "user", "#{remittent.user.name}","pic", "#{get_user_photo(remittent.user)}" , "description", "#{remittent.description}", "packege_img", "#{get_package_photo(remittent)}", "location_arrived", 
